                    ...
                    let form = new ui.MessageFormData()
                        ...
                        .show(player).then(res => {
                            if (res.selection === 1) {
                                ...
                                cmd(`scoreboard players reset "${name}" lands`).then(() => {
                                    ...
                                    for (let pl of mc.world.getAllPlayers()) {
                                        for (let tag of pl.getTags()) {
                                            if (tag.includes(JSON.stringify(land))) {
                                                pl.removeTag(tag)
        ...
        function nether () {
            ...
            function add() {
                ...
                form.show(player).then(res => {
                    ...
                    let name = res.formValues[0].trim()
                    let playerLands = getPlayerLands(player, 'nether')
                    if (name == '') {
                        return logfor(player.name, `§c§l>> §e領地名稱不得為空!`)
                    }
                    for (let land of playerLands) {
            ...
            function remove() {
                if (getPlayerLands(player, 'nether').length == 0) {
                    return logfor(player.name, '§c§l>> §e您還沒有領地!')
                }
                let lands = getPlayerLands(player, 'nether')
                let form = new ui.ActionFormData()
                    .title("§c§l刪除領地")
                for (let land of lands) {
                    let x1 = Math.max(land.pos.x[1], land.pos.x[2])
                    ...
                    let form = new ui.MessageFormData()
                        ...
                        .show(player).then(res => {
                            if (res.selection === 1) {
                                let name = transfromLand(land)
                                let squ = (Math.abs(Number(land.pos.x[1]) - Number(land.pos.x[2])) + 1) * (Math.abs(Number(land.pos.z[1]) - Number(land.pos.z[2])) + 1)
                                cmd(`scoreboard players reset "${name}" lands_nether`).then(() => {
                                    player.runCommandAsync(`scoreboard players add @s "land_squ" -${squ}`)
                                    player.runCommandAsync(`scoreboard players add @s "land_land" -1`)
                                    for (let pl of mc.world.getAllPlayers()) {
                                        for (let tag of pl.getTags()) {
                                                ...
                                                if (res.selection === 0) {
                                                            ...
                                                            .show(player).then(res => {
                                                                if (res.selection === 0) {
                                                                            ...
                                                                            if (res.selection === 0) {
                                                                                let form = new ui.ModalFormData()
                                                                                    ...
                                                                                    .show(player).then(res => {
                                                                                        ...
                                                                                        cmd(`scoreboard players reset "${transfromLand(land)}" lands_nether`)
                                                                                        land.users.push({ username: name, permission: { build: "false", container: "false", action: "true" } })
                                                                                        cmd(`scoreboard players set "${transfromLand(land)}" lands_nether ${land.UID}`).then(() => {
                                                                                            logfor(player.name, '§a§l>> §e設定成功!')
                                                                                            return Personal();
                                                                                        })
                                                                                    })
                                                                    ...
                                                                    form.show(player).then(res => {
                                                                        ...
                                                                        let newUsers = land.users.filter(item => item.username !== selePlayer.username);
                                                                        land.users = newUsers
                                                                        cmd(`scoreboard players set "${transfromLand(land)}" lands_nether ${land.UID}`)
                                                                        logfor(player.name, `§a§l>> §e刪除成功!`)
                                                                    })
                                                                } else if (res.selection === 2) {
                                                                    permissionChange()
                                                    ...
                                                    let form = new ui.ModalFormData()
                                                        ...
                                                        .show(player).then(res => {
                                                            ...
                                                            let build = res.formValues[0]
                                                            let container = res.formValues[1]
                                                            cmd(`scoreboard players reset "${transfromLand(land)}" lands_end`)
                                                            land.permission.build = `${build}`
                                                            land.permission.container = `${container}`
                                                            cmd(`scoreboard players set "${transfromLand(land)}" lands_end ${land.UID}`)
                                                            logfor(player.name, `§a§l>> §e修改成功!`)
                                                            return perMenu()
                                                        })
                                                } else if (res.selection === 2) {
        ...
        function end () {
            ...
            function add() {
                ...
                form.show(player).then(res => {
                    ...
                    let name = res.formValues[0].trim()
                    let playerLands = getPlayerLands(player, 'end')
                    if (name == '') {
                        return logfor(player.name, `§c§l>> §e領地名稱不得為空!`)
                    }
                    for (let land of playerLands) {
            ...
            function remove() {
                if (getPlayerLands(player, 'end').length == 0) {
                    return logfor(player.name, '§c§l>> §e您還沒有領地!')
                }
                let lands = getPlayerLands(player, 'end')
                let form = new ui.ActionFormData()
                    .title("§c§l刪除領地")
                for (let land of lands) {
                    let x1 = Math.max(land.pos.x[1], land.pos.x[2])
                    ...
                    let form = new ui.MessageFormData()
                        ...
                        .show(player).then(res => {
                            if (res.selection === 1) {
                                let name = transfromLand(land)
                                let squ = (Math.abs(Number(land.pos.x[1]) - Number(land.pos.x[2])) + 1) * (Math.abs(Number(land.pos.z[1]) - Number(land.pos.z[2])) + 1)
                                cmd(`scoreboard players reset "${name}" lands_end`).then(() => {
                                    player.runCommandAsync(`scoreboard players add @s "land_squ" -${squ}`)
                                    player.runCommandAsync(`scoreboard players add @s "land_land" -1`)
                                    for (let pl of mc.world.getAllPlayers()) {
                                        for (let tag of pl.getTags()) {
                                                                            ...
                                                                            if (res.selection === 0) {
                                                                                let form = new ui.ModalFormData()
                                                                                    ...
                                                                                    .show(player).then(res => {
                                                                                        ...
                                                                                        cmd(`scoreboard players reset "${transfromLand(land)}" lands_end`)
                                                                                        land.users.push({ username: name, permission: { build: "false", container: "false", action: "true" } })
                                                                                        cmd(`scoreboard players set "${transfromLand(land)}" lands_end ${land.UID}`).then(() => {
                                                                                            logfor(player.name, '§a§l>> §e設定成功!')
                                                                                            return Personal();
                                                                                        })
                                                                                    })
